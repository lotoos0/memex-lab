    ...
    *,
    ...
    if quality_band == "partial":
        if label == "interesting":
            return "review_now", ["quality_band_partial", "label_interesting_promoted"], True
        return "review_if_time", ["quality_band_partial"], False
...
def build_label_map(label_records: list[dict[str, Any]]) -> dict[str, dict[str, str]]:
    ...
    for record in label_records:
        ...
        if mint and label and labeled_at:
            entry = {
                "mint": mint,
                "label": label,
